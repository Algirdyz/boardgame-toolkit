...
function Templates() {
  ...
  if (templates.isLoading) return <Center><Loader /></Center>
  if (templates.isError) return <div>Error loading templates</div>


  const cards: NavigationCard[] = templates.data ? templates.data.map(t => ({
    title: t.name,
    link: `/templates/${t.id}`,
  })) : [];
  ...
  return (
    <Container>
      <Modal opened={opened} onClose={close} title="New Template">
        <TextInput label="Template Name" placeholder="Enter template name" mb="md" value={name} onChange={(e) => setName(e.currentTarget.value)} />
        <Button onClick={async () => {
          const defaultTemplate: TemplateDefinition = {
            name,
            shape: [{ x: 0, y: 0 }],
            workerDefinition: { maxCount: 5, rows: 1, spacing: 10, workerSlotPositions: { x: 10, y: 10 } },
            nameDefinition: { position: { x: 10, y: 50 }, maxWidth: 100 },
            resourceListDefinition: { resources: [], spacing: 10, position: { x: 10, y: 80 } },
          }
      ...
      <Button mb="md" onClick={open} leftSection={<IconPlus />}>Create New Template</Button>
