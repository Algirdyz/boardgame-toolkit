...
import { useNameDef } from '../tileComponents/nameTag/Name';
import { useResourceListDef } from '../tileComponents/resourceCost/ResourceList';
import { useWorkerDef } from '../tileComponents/workerSlots/WorkerSlots';
import drawGrid from '@/components/canvas/canvasGrid';
import { PIXELS_PER_CM } from '@/lib/constants';
import { getDPI } from '@/lib/dpi';
...
const TemplateCanvas = ({ template, onTemplateChange }: TemplateCanvasProps) => {
  ...
  const name = useNameDef(canvasRef.current, (nameDef) => {
    const updatedTemplate = { ...template, nameDefinition: nameDef };
    onTemplateChange(updatedTemplate);
  });
  const resourceList = useResourceListDef(canvasRef.current, (resourceListDef) => {
    const updatedTemplate = { ...template, resourceListDefinition: resourceListDef };
    onTemplateChange(updatedTemplate);
  });
  ...
  useEffect(() => {
    ...
    // Add template shape
    const shape = getShape(type, '#d3d3d3ff');
    const lastShape = canvas.getObjects().find((obj) => obj.id === 'shape');
    if (lastShape) canvas.remove(lastShape);
    shape.set({
      left: pixelsPerCm,
      top: pixelsPerCm,
      selectable: false,
